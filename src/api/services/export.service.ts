...
class Export extends BaseService {
  public async export(path: string, ext: ExportFormat): Promise<ExportDTO> {
    const response = await window.electron.ipcRenderer.invoke(IpcEvents.EXPORT, path, ext);
    return this.response(response);
  }

  public async notarizeSBOM(args: string | null = null): Promise<any> {
    const response = await window.electron.ipcRenderer.invoke(IpcEvents.EXPORT_NOTARIZE_SBOM, args);
    return this.response(response);
  }
}

...
export async function migration110(projectPath: string): Promise<void> {
  log.info('%cMigration 1.1.0 In progress...', 'color:green');
  ...
  const result: Record<any, any> = await utilModel.readFile(
    `${projectPath}/result.json`
  );
    ...
    try {
      const db: any = new sqlite3.Database(
        ...
        (err: any) => {
          ...
          db.run(
            'ALTER TABLE component_versions ADD COLUMN reliableLicense varchar(100) DEFAULT NULL;'
          );
          db.run(
...
async function insertResultLicense(
  projectPath: string,
  filesResult: Array<any>,

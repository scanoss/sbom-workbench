...
LEFT JOIN license_view lic ON comp.id=lic.cvid
 ...
 ON counter.purl=comp.purl AND counter.version=comp.version;`;
  ...
  SQL_GET_RESULTS_IN_FOLDER = `SELECT id,identified,ignored,(CASE WHEN  identified=0 AND ignored=0 THEN 1 ELSE 0 END) AS pending FROM results WHERE file_path LIKE '?' AND source='engine';`;
}

...
LEFT JOIN license_view lic ON comp.id=lic.cvid
 ...
 ON counter.purl=comp.purl AND counter.version=comp.version;`;
  ...
  SQL_GET_RESULTS_RESCAN = `SELECT r.file_path as path,r.identified ,r.ignored ,(CASE WHEN  r.identified=0 AND r.ignored=0 THEN 1 ELSE 0 END) as pending FROM results r;`;
}

export class Querys {
  ...
  SQL_CREATE_TABLE_INVENTORY =
    'CREATE TABLE IF NOT EXISTS inventories (id INTEGER PRIMARY KEY ,version TEXT NOT NULL ,compid INTEGER NOT NULL,purl TEXT, usage TEXT, notes TEXT, url TEXT, spdxid TEXT, FOREIGN KEY (compid) REFERENCES component_versions(id) ON  DELETE CASCADE );';
  ...
  FROM inventories i INNER JOIN component_versions cv ON cv.id=i.compid;`;
  ...
  SQL_SCAN_SELECT_INVENTORIES_FROM_PURL_VERSION = `SELECT i.id,i.compid,i.usage,i.notes,i.url,i.spdxid,i.purl,i.version,l.name AS license_name FROM inventories i INNER JOIN component_versions cv ON i.compid=cv.id INNER JOIN licenses l ON i.spdxid=l.spdxid WHERE cv.purl=? AND cv.version=?;`;

  // GET INVENTORY BY ID
  SQL_GET_INVENTORY_BY_PURL = 'SELECT i.id,i.compid,i.usage,i.notes,i.url,i.spdxid,i.purl,i.version,l.name AS license_name FROM inventories i INNER JOIN licenses l ON i.spdxid=l.spdxid INNER JOIN component_versions cv ON cv.id=i.compid WHERE cv.purl=?;';
  ...
  FROM inventories  i 
  INNER JOIN license_view l ON l.cvid=i.compid 
  INNER JOIN component_versions c ON c.id=i.compid 
  ...
  LEFT JOIN results r ON r.id=fi.resultid INNER JOIN component_versions cv ON cv.id=i.compid;`;

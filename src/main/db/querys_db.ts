...
LEFT JOIN license_view lic ON comp.id=lic.cvid
 ...
 ON counter.purl=comp.purl AND counter.version=comp.version;`;
  ...
  SQL_GET_RESULTS_SUMMARY = `SELECT filesIdentified.identified,filesPending.pending,filesOriginal.original,filesNoMatch.noMatch,filesFiltered.filtered,filesMatch.match,totalFiles.totalFiles, scannedFiles.scannedFiles FROM ((SELECT COUNT(*) AS identified FROM files f WHERE f.identified=1) AS filesIdentified,

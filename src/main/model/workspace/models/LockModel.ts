...
export class LockModel {
  ...
  public async get(projectPath: string, username: string, hostname:string): Promise<Lock>{
    ...
    return await call(`SELECT l.project, l.username, l.hostname, l.createdAt , l.updatedAt FROM lock as l WHERE l.project = ? AND l.username = ? AND l.hostname = ?;`, projectPath, username, hostname);  // filter by projectName
  }
  ...
  public async create(data: { projectPath:string, username:string, hostname:string }): Promise<Lock> {
    const call:any = util.promisify(this.connection.get.bind(this.connection));
    await call('INSERT INTO lock (project, username, hostname, createdAt, updatedAt) values(?,?,?,?,?);', data.projectPath, data.username, data.hostname, new Date().toISOString(), new Date().toISOString());
  ...
  public async update(data: { projectPath: string, username: string, hostname: string }): Promise<Lock> {
    const call:any = util.promisify(this.connection.get.bind(this.connection));
    await call('UPDATE lock SET  username=? , hostname=?, createdAt=?, updatedAt=? WHERE project = ?;', data.username, data.hostname, data.createdAt, data.updatedAt, data.projectPath);
    return this.get(data.projectPath, data.username, data.hostname);
  }

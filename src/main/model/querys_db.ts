...
export class Queries {
  ...
  SQL_CREATE_TABLE_RESULTS =
    'CREATE TABLE IF NOT EXISTS results (id integer primary key asc,md5_file text,fileId integer, vendor text, component text, version text, latest_version text, cpe text, url text, lines text, oss_lines text, matched text, filename text, size text, idtype text, url_hash varchar(32) ,compid integer,purl text,file_url text,source text,dirty INTEGER default 0,download_url varchar(128) DEFAULT NULL, FOREIGN KEY (fileId) REFERENCES files(fileId));';
  ...
  FILES_TABLE =
    'CREATE TABLE IF NOT EXISTS files (fileId INTEGER PRIMARY KEY ASC,path TEXT UNIQUE,md5_file TEXT,identified INTEGER DEFAULT 0, ignored INTEGER DEFAULT 0, dirty INTEGER DEFAULT 0, type TEXT);';
  ...
  WORKSPACE_DB = this.WORKSPACE_LOCK + this.WORKSPACE_SEARCH_ITEM_GROUP_TABLE;

  /** SQL SCAN INSERT* */
  // SQL INSERT RESULTS
  SQL_INSERT_RESULTS =
    'INSERT or IGNORE INTO results (md5_file,vendor,component,version,latest_version,url,lines,oss_lines,matched,filename,idtype,url_hash,purl,fileId,file_url,source, download_url) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
  ...
  COMPDB_SQL_COMP_VERSION_CREATE =
    'INSERT INTO component_versions (name,version, description, url,purl,source) VALUES (?,?,?,?,?,?);';
  ...
  // GET LICENSES
  SQL_SELECT_ALL_LICENSES_FULL_TEXT =
    'SELECT id, spdxid, name, url, official, fulltext FROM licenses ORDER BY name ASC;';
  ...
  SQL_SELECT_INVENTORIES_NOT_HAVING_FILES =
    "SELECT i.id FROM inventories i  WHERE i.id NOT IN (SELECT inventoryid FROM file_inventories) AND i.source='detected';";
  ...
  SQL_GET_SUMMARY_BY_RESULT_ID =
    'SELECT f.path,f.identified ,f.ignored ,(CASE WHEN  f.identified=0 AND f.ignored=0 THEN 1 ELSE 0 END) as pending FROM files f  WHERE fileId IN #values GROUP BY f.path;';
...
FROM files f LEFT JOIN results r ON (r.fileId=f.fileId) #FILTER ;`;
 ...
 algorithms_count AS (
  ...
  SQL_DETECTED_REPORT_LICENSE_COMPONENT_SUMMARY = `
    SELECT spdxid, SUM(detectedLicenseComponentCount) as componentLicenseCount, SUM(declaredLicenseDependencyCount) as dependencyLicenseCount , SUM(detectedLicenseComponentCount + declaredLicenseDependencyCount) as total FROM (
    ...
    WHERE cv.source = 'engine' AND spdxid NOT NULL
    GROUP BY l.spdxid
    UNION
-- Second part: splitting originalLicense by ',' and counting dependency licenses
    ...
    WITH dependencyLicense as (
    SELECT
    d.purl,
    d.version,
    COALESCE(l.value, 'unknown')  as spdxid
    FROM dependencies d,
    json_each('["' || REPLACE(d.originalLicense, ',', '","') || '"]') AS l)
    SELECT spdxid,purl,version
    FROM dependencyLicense
    WHERE spdxid != ''

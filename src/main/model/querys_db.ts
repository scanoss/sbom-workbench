...
WHERE f.fileId IN (SELECT fileId FROM results) OR f.fileId IN (SELECT fileId FROM dependencies);`;
  ...
  SQL_GET_ALL_FILES = `SELECT f.path,f.fileId AS id,f.identified,f.ignored,(CASE WHEN f.identified=0 AND f.ignored=0 THEN 1 ELSE 0 END) AS pending, f.type FROM files f  #FILTER;`;

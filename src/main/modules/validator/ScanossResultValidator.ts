...
export class ScanossResultValidator {
 ...
 private  validateFileResult(prefix, result) {
    ...
    if (!result.url_stats || typeof result.url_stats !== 'object' || Array.isArray(result.url_stats)) {
      this.addError(`${prefix}.url_stats`, 'required field must be an object', result.url_stats);
    }
  }

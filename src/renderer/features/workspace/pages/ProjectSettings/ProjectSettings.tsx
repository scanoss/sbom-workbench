...
import { Alert, Button, Checkbox, FormControlLabel, FormHelperText, IconButton, Link, MenuItem, Paper, Select, TextField, Tooltip } from '@mui/material';
...
import ProxyConfigSetup from './components/ProxyConfigSetup';
...
const ProjectSettings = () => {
  ...
  const onDecompressHandler = (checked: boolean) => {
    const newType = projectSettings.scannerConfig.type.filter((type) => type !== ScannerType.UNZIP);
        ...
        <form onSubmit={(e) => handleClose(e)}>
          <div className="grid-layout form-container mt-1">
            <div className="d-flex flex-column gap-5">
              ...
              <ProxyConfigSetup projectSettings={projectSettings} setProjectSettings={setProjectSettings} />
              ...
              <div>
                ...
                <FormGroup className="ml-3">
                  ...
                  />
                  <FormHelperText className="helper">{t('DecompressArchivesHint')}</FormHelperText>
                </FormGroup>
                ...
                <FormGroup className="ml-3">
                  ...
                  />
                  <FormHelperText className="helper">{t('ObfuscateFilePathsHint')}</FormHelperText>
                </FormGroup>
                ...
                <FormGroup className="ml-3">
                  ...
                  />
                  <FormHelperText className="helper">{t('HPSMHint')}</FormHelperText>
                </FormGroup>
              </div>
            </div>
            ...
            <div className="grid-item">
              <div className="context-files-info">
                {context && !context.identifyFile && !context.ignoreFile && (
                  ...
                  >
                    No context file found. You can provide one by creating an SBOM file in the{' '}
                    <Link className="cursor-pointer" color="inherit" onClick={onOpenWorkRoot}>
                ...
                {context && context.identifyFile && (
                  <Alert severity="success">
                    A context file was found (
                    <Link className="cursor-pointer" color="inherit" onClick={() => onOpenFile(context.identifyFile)}>
                      {context.identifyFile}
                    </Link>
                    ). It will be used to enhance the scan results.
                  </Alert>
                )}

                {context && context.ignoreFile && (
                  <Alert severity="success">
                    An ignore file was found (
                    <Link className="cursor-pointer" color="inherit" onClick={() => onOpenFile(context.ignoreFile)}>
                      {context.ignoreFile}
                    </Link>
                    ). It will be used to avoid specific results.
                  </Alert>
                )}

                {context && context.identifyFile && context.ignoreFile && <Alert severity="warning">The identified context file takes precedence over the ignore file.</Alert>}
              </div>
            </div>
          </div>

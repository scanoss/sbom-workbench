...
const ProjectSettings = () => {
  ...
  const onPipelineStageHandler = (type: ScannerType, checked: boolean) => {
    ...
    // When source is CODE, at least one of CODE or DEPENDENCIES must remain enabled
    if (scanPath?.source === ScannerSource.CODE && !checked
      && (type === ScannerType.CODE || type === ScannerType.DEPENDENCIES)
      && !newType.includes(ScannerType.CODE) && !newType.includes(ScannerType.DEPENDENCIES)) {

...
export const PreLoadInventoryDialog = (props: IPreLoadInventoryDialog) => {
  ...
  const init = async () => {
    ...
    const inv = response.sort((a, b) => {
      if (a.spdxid === '') return 1;
      if (b.spdxid === '') return -1;
        ...
        <DialogContent>
          <form onSubmit={onSubmit}>
            <div>
              <List>
                <FormControlLabel
                  control={<Checkbox checked={AllChecked()} onClick={() => selectAll()} />}
                  label="All"
                />
                              ...
                              <div className="name-component">
                                ...
                                >
                                  <path
                                    ...
                                    fill={value.spdxid === '' ? '#7E7E7E' : '#3B82F6'}
                                  />
                                </svg>

                                <p className={value.spdxid === '' ? 'list-item-text-no-license' : 'list-item-text'}>
                                  {`${value.purl}`}
                                </p>
                              </div>
                              <div className="pills">
                                {value.spdxid === '' ? (
                                  <div className="version-pill-no-license">

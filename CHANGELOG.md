...
## [Unreleased]
### Fixed
- Fixed WFP generation for binary files
...
## [1.33.1] - 2026-02-12
### Fixed
- Fixed duplicate file entries in the database by adding a UNIQUE constraint on the `path` column in the `files` table
- Updated file insert query to use `ON CONFLICT(path) DO UPDATE` instead of `INSERT OR REPLACE` to preserve existing `fileId` references
### Added
- Added migration script (1.33.1) to remove duplicate file records and recreate the `files` table with a UNIQUE constraint on `path`
